#!/bin/bash
# runLocal.sh — Run gatk.FilterMutectCalls locally in Docker using gpunit test data.
...
WRAPPER="${MODULE_DIR}/gatk_filtermutectcalls_wrapper.sh"

echo "=== gatk.FilterMutectCalls local Docker test run ==="
echo "=== Image:   ${IMAGE}"
echo "=== Data:    ${DATA_DIR}"
echo "=== Output:  ${RUN_DIR}"
echo ""
...
CMD=(docker run --rm
  -v "${DATA_DIR}:/data"
  -v "${RUN_DIR}:/work"
  -v "${WRAPPER}:/usr/local/bin/gatk_filtermutectcalls_wrapper.sh"
  -w /work
  "${IMAGE}"
  bash /usr/local/bin/gatk_filtermutectcalls_wrapper.sh
